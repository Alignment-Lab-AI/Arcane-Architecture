...
#Batch normalization layers can be added to regularize and stabilize the training process.
#Layer normalization can be used in conjunction with the dropout layers to further improve the model's 
#training stability and generalization capabilities.
#The model can be improved by employing attention mechanisms, as used in the original Transformer model,
#to facilitate better understanding of context and dependencies in the input data.
#Gradient clipping can be used to prevent exploding gradients during training.
#Mixed precision training can be deployed to speed up the training process and reduce memory usage.
#Weight sharing between layers can be employed to reduce the number of parameters and memory usage.
#potentially able to be adapted for transformers architecture modifications
